...
trainer = TrainerHL(model = model, data = data, train_times = 50,opt_method = "Adam", alpha = 0.0003, use_gpu = True)
trainer.run(r_h=r_h,r_t=r_t,r_r=r_r)
transh_dem.save_checkpoint('./checkpoint/H_DEM.ckpt')


# transh_dem.load_checkpoint('./checkpoint/H_DEM.ckpt')
# tester = Tester(model = transh_dem, data_loader = test_dataloader, use_gpu = True,
# 				path2="result/FB15K/H_DEM.csv"
# 				)
